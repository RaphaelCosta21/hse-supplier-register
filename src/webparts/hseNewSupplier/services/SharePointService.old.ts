...
export class SharePointService {
  ...
  constructor(context: WebPartContext, listName: string) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Lista ${this.listName} criada com sucesso`);
      } catch (createError) {
        console.error(`Erro ao criar lista ${this.listName}:`, createError);
        // Se falhar, tentar uma abordagem mais simples - apenas criar a lista sem campos customizados
        try {
          console.log("Tentando criar lista básica sem campos customizados...");
          await this.sp.web.lists.add({
            Title: this.listName,
            Description: "Lista para formulários HSE de fornecedores",
            BaseTemplate: 100
          });
          console.log("Lista básica criada com sucesso");
        } catch (fallbackError) {
          console.error("Erro ao criar lista básica:", fallbackError);
          throw fallbackError;
        }
      }
    }
  ...
  private async createHSEList(): Promise<void> {
    ...
    // Adicionar apenas campos essenciais de forma mais simples
    await this.addBasicFields(list);
  }

  private async addBasicFields(list: any): Promise<void> {
    // Usar apenas campos de texto e nota para evitar problemas de tipos
    const textFields = [
      "Empresa",
      "CNPJ", 
      "NumeroContrato",
    ...
    for (const fieldName of textFields) {
      try {
        ...
        console.log(`Campo de texto ${fieldName} criado com sucesso`);
      } catch (error) {
        console.error(`Erro ao criar campo de texto ${fieldName}:`, error);
      }
    }

    // Adicionar campos de nota (texto longo)
    for (const fieldName of noteFields) {
      try {
        await list.fields.addMultilineText(fieldName);
        console.log(`Campo de nota ${fieldName} criado com sucesso`);
      } catch (error) {
        console.error(`Erro ao criar campo de nota ${fieldName}:`, error);
      }
    }

    // Adicionar campos numéricos
    try {
      await list.fields.addNumber("PercentualConclusao");
      console.log("Campo PercentualConclusao criado com sucesso");
    } catch (error) {
      console.error("Erro ao criar campo PercentualConclusao:", error);
    }

    try {
      await list.fields.addNumber("AnexosCount");
      console.log("Campo AnexosCount criado com sucesso");
    } catch (error) {
      console.error("Erro ao criar campo AnexosCount:", error);
    }

    // Adicionar campo de data
    try {
      await list.fields.addDateTime("DataEnvio");
      console.log("Campo DataEnvio criado com sucesso");
    } catch (error) {
      console.error("Erro ao criar campo DataEnvio:", error);
    }
  }
  ...
  public async saveFormData(
    ...
    const calculateCompletionPercentage = (): number => {
      ...
      if (formData.conformidadeLegal && Object.keys(formData.conformidadeLegal).length > 0) completed++;
